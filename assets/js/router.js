window.router = ((win, doc) => {
    if ("onhashchange" in window) {
        ...
        let loadView = () => doc.querySelector("content");
        ...
        let cleanRouterView = () => doc.querySelector("content").innerHTML = "";
        let init = (rutas) => {
                ...
                for (_index in configRouter.route) {
                    if (location.hash.replace("#", "/") === configRouter.route[_index].path) {
                        ...
                        if (configRouter.route[_index].view.indexOf("#") >= 0) {
                            ...
                            history.pushState(null, "", configRouter.route[_index].path);
